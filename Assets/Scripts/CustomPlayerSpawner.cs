...
public class CustomPlayerSpawner : MonoBehaviour, INetworkRunnerCallbacks
{
	...
	public void OnObjectWordsChanged(NetworkRunner runner, NetworkObject networkedObject, HashSet<int> changedWords,
	...
	public async void StartGame(GameMode mode)
	{
		...
		await _runner.StartGame(new StartGameArgs() {GameMode = mode, SessionName = "TestRoom"});

		// If we are the host, also select the current Unity scene as the active Fusion scene
	...
	private bool TryGetSceneRef(out SceneRef sceneRef)
	{
		...
		return config.TryGetSceneRef(scenePath, out sceneRef);
	}
